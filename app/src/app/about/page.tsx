...
const REALITY: ReadonlyArray<{ component: string; state: RealityState; note: string }> = [
  ...
  { component: "RA-quote-bound attestationDigest in receipts", state: "draft", note: "/api/consume wired to Phala TDX enclave (composeHash 0x1884…cea0, signer 0xaba6…a7e7); when reachable the digest is sha256 of canonicalized receipt + per-receipt TDX quote whose report_data binds (signer, image, receiptId). Probe /api/tee-status for live state — when the enclave is unreachable the route falls back to a named stub digest" },
