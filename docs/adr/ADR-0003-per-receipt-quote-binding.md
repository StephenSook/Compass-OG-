...
- **Verifier in three places is the same code.** Browser ([`app/src/lib/verifyReceipt.ts`](../../app/src/lib/verifyReceipt.ts)), CLI ([`enclave/scripts/verify-receipt.ts`](../../enclave/scripts/verify-receipt.ts)), and Claude Code skill ([`skills/compass-eligibility-check/`](../../skills/compass-eligibility-check/SKILL.md)) all run the same four checks. Byte-parity is enforced by a canonicalization function shared between Node and browser ports (see [`app/src/lib/verifyReceipt.ts`](../../app/src/lib/verifyReceipt.ts) `canonicalize()`).
