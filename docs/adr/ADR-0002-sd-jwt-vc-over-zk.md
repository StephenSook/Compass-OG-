...
- **Rejected because:** Adopting BBS+ in May 2026 means betting on a draft that is still pre-final; the pairing-curve dependency makes the browser-side verifier (`app/src/lib/verifyReceipt.ts`) ~3× heavier than the `@noble/curves` secp256k1 + Ed25519 stack we already need; unlinkability is **not the threat model** for Compass — replay protection is enforced by `buildNullifier(nonce, provider, policy)` against `CompassHub.usedNullifiers`, which does *not* require a stable `agentIdCommitment`. Compass keeps `agentIdCommitment` as a per-(holder, provider, policy, epoch)-scoped commitment for audit-trail composition, accepting that linkability across receipts of the same holder is *expected* in this design. The nullifier mechanism and the commitment are orthogonal — see `enclave/src/buildNullifier.ts` + `contracts/CompassHub.sol` for the replay path.
